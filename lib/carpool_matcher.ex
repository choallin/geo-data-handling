defmodule CarpoolMatcher do
  ...
  def find_boxes(origin, destination) do
    bbox =
      Agent.get(__MODULE__, fn boxes ->
        Enum.flat_map(boxes, fn box ->
          # This case happens when both points are in the same box
          if GeoDataHandling.is_coordinate_in_bounding_box?(box, origin) &&
               GeoDataHandling.is_coordinate_in_bounding_box?(box, destination) do
